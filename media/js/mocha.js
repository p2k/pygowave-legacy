...
Options:
	...
	buttons - (object) Window buttons; either an Element or an array with objects of this structure: {label: (string), onClick: (function)}
	rtl - (boolean) True if right to left (affects title bar)
...
MochaUI.Window = new Class({
	...
	*/
	newWindow: function(properties){ // options is not doing anything
		...
		if ($type(this.options.buttons) == 'object') {
			$each(this.options.buttons, function (f, label) {
				new Element("button", {
					"type": "button",
					"class": "mochaButton"
				})
				.set('text', label)
				.addEvent('click', f.bind(this.windowEl))
	...
	*/
	insertWindowElements: function(){
		...
		if ($type(options.buttons) == 'object' || $type(options.buttons) == 'element') {
			...
			cache.buttonsEl = new Element('div', {
				'id': id + '_buttons',
				'class': 'mochaButtonbox'
			}).injectInside(cache.buttonsWrapperEl);
		}
	...
	*/	
	...
	drawControls : function(width, height, shadows){
		...
		this.controlsEl.setStyle('top', shadowBlur - shadowOffset.y + controlsOffset.top);
		if (options.rtl)
			this.controlsEl.setStyle('left', shadowBlur + shadowOffset.x + controlsOffset.right);
		else
			this.controlsEl.setStyle('right', shadowBlur + shadowOffset.x + controlsOffset.right);

		this.canvasControlsEl.setStyle('top', shadowBlur - shadowOffset.y + controlsOffset.top);
		if (options.rtl)
			this.canvasControlsEl.setStyle('left', shadowBlur + shadowOffset.x + controlsOffset.right);
		else
			this.canvasControlsEl.setStyle('right', shadowBlur + shadowOffset.x + controlsOffset.right);
