...
/*!
 * MooTools Corners 0.1
 * Copyright (c) 2009 Patrick Schneider
 * 
 * Port of jQuery Corners 0.3
 * Copyright (c) 2008 David Turnbull, Steven Wittens
 * Dual licensed under the MIT (MIT-LICENSE.txt)
 * and GPL (GPL-LICENSE.txt) licenses.
 */
 
Elements.implement({corners: function(options) {
  var doneClass = 'rounded_by_mootools_corners'; /* To prevent double rounding */
  ...
  function roundWebkit(e, s) {
    ...
    var $e = $(e);
    if (s.tl) $e.setStyle('WebkitBorderTopLeftRadius', radius);
    if (s.tr) $e.setStyle('WebkitBorderTopRightRadius', radius);
    if (s.bl) $e.setStyle('WebkitBorderBottomLeftRadius', radius);
    if (s.br) $e.setStyle('WebkitBorderBottomRightRadius', radius);
  }
  ...
  {  
    ...
    var $e = $(e);
    if (s.tl) $e.setStyle('-moz-border-radius-topleft', radius);
    if (s.tr) $e.setStyle('-moz-border-radius-topright', radius);
    if (s.bl) $e.setStyle('-moz-border-radius-bottomleft', radius);
    if (s.br) $e.setStyle('-moz-border-radius-bottomright', radius);  
  }
  ...
  function roundLink(e, s, bgColor, fgColor) {
    ...
    if (Browser.Engine.trident) table.onclick = ieLinkBypass;
  ...
  function changeInput(e) {
    ...
    if (e.onclick) {
      ...
      $each($(e).getParent('form'), function(item) {a1.href = item.action;}.bind(this));
  ...
  function submitForm() {
    $each($(this).getParent('form'), function(item) {item.submit()}.bind(this));
    return false;
  }
  ...
  function relocateContent(e, s, d) {
    ...
    if (e.style.height) {
      var h = parseInt($e.getStyle('height'));
      d.style.height = h + 'px';
      h += parseInt($e.getStyle('padding-top')) + parseInt($e.getStyle('padding-bottom'));
      e.style.height = h + 'px';
    }
    if (e.style.width) {
      var w = parseInt($e.getStyle('width'));
      d.style.width = w + 'px';
      w += parseInt($e.getStyle('padding-left')) + parseInt($e.getStyle('padding-right'));
      e.style.width = w + 'px';
    }
    d.style.paddingLeft = $e.getStyle('padding-left');
    d.style.paddingRight = $e.getStyle('padding-right');
    if (s.tl||s.tr) {
      d.style.paddingTop = adjustedPadding(e, s, $e.getStyle('padding-top'), true);
    } else {
      d.style.paddingTop = $e.getStyle('padding-top');
    }
    if (s.bl||s.br) {
      d.style.paddingBottom = adjustedPadding(e, s, $e.getStyle('padding-bottom'), false);
    } else {
      d.style.paddingBottom = $e.getStyle('padding-bottom');
    }
    e.style.padding = 0;
    return d;
  }
  ...
  function parseOptions(options, settings) {
    ...
    $each(options.split(' '), function(option, idx) {
